...
class MultiCollisionDataset(PointCloudDataset):
    """Class to handle MultiCollision dataset."""
    ...
    def __init__(self,
                 ...
                 load_data=True):
        PointCloudDataset.__init__(self, 'MultiCollision')
        ...
        # Original data path (multiple paths)
        self.original_path = dataset_paths
        ...
        # Get full sequence path as they come from differnt folders
        self.sequences = []
        self.data_path = []
        self.seq_path = []
        self.colli_path = []
        self.annot_path = []
        self.frames = []
        for data_path, day_list, sim in zip(dataset_paths, day_lists, simulated):
            if not sim:
                for seq in day_list:
                    self.sequences.append(seq)
                    self.data_path.append(data_path)
                    self.seq_path.append(join(data_path, 'runs', seq, 'velodyne_frames'))
                    self.colli_path.append(join(data_path, 'collisions', seq))
                    self.annot_path.append(join(data_path, 'annotated_frames', seq))
        ...
        if np.sum(np.array(simulated).astype(np.int32)) > 1:
            raise ValueError('Multiple simulated dataset not handled')
            
        if np.any(simulated):

            sim_ind = np.where(simulated)[0][0]
            add_sim_path = dataset_paths[sim_ind]
            add_sim_days = day_lists[sim_ind]
        ...
        if self.set in ['training', 'validation']:
            ...
            for s_ind, seq in enumerate(self.sequences):
                ...
                if self.sim_sequences[s_ind]:
                    ...
                    data_path = self.data_path[s_ind]
...
class MultiCollisionSampler(Sampler):
    """Sampler for MultiCollision"""

    def __init__(self, dataset: MultiCollisionDataset, manual_training_frames=False, debug_custom_inds=False):
...
class MultiCollisionSamplerTest(MultiCollisionSampler):
    """Specific Sampler for MultiCollision Tests which limits the predicted frame indices to certain ones"""

    def __init__(self, dataset: MultiCollisionDataset, test_ratio=0.1, wanted_frame_inds=[]):
        MultiCollisionSampler.__init__(self, dataset)
...
class MultiCollisionCustomBatch:
    """Custom batch definition with memory pinning for MultiCollision"""
...
def MultiCollisionCollate(batch_data):
    return MultiCollisionCustomBatch(batch_data)
